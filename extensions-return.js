...
export const ProductSelectionExtension = {
  name: 'ProductSelection',
  type: 'response',
  match: ({ trace }) =>
    trace.type === 'ext_productSelection' || trace.payload.name === 'ext_productSelection',
  render: ({ trace, element, context }) => {
    const productSelectionContainer = document.createElement('div');
    productSelectionContainer.innerHTML = `<style>
      .my-product-selection button {
        margin: 5px;
        padding: 10px;
    ...
    <div class='my-product-selection'></div>`;

    const productSelectionBox = productSelectionContainer.querySelector('.my-product-selection');

    // Functie om knoppen te genereren op basis van returnProductNames
    const generateProductButtons = () => {
      const numberOfProducts = Object.keys(context).filter(key => key.startsWith('returnProductName_')).length;

      for (let i = 0; i < numberOfProducts; i++) {
        const productName = context[`returnProductName_${i}`];
        const button = document.createElement('button');
        button.textContent = productName;
        button.addEventListener('click', () => {
          console.log(`${productName} selected for return`);

          // Implementeer de logica voor wat er moet gebeuren als een product wordt geselecteerd
          // Bijvoorbeeld, stuur een bericht terug naar de server of update de UI
        });

        productSelectionBox.appendChild(button);
      }
    };

    generateProductButtons();

    element.appendChild(productSelectionContainer);
  },
};