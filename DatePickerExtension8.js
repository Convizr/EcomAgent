window.addEventListener('message', (event) => {
    const allowedOrigin = 'https://general-runtime.voiceflow.com';

    if (event.origin !== allowedOrigin) {
        // Als de origin van het bericht niet overeenkomt met wat je verwacht, negeer het bericht dan.
        console.error('Received message from untrusted origin:', event.origin);
        return;
    }

    try {
        // Als de origin overeenkomt, ga je door met het verwerken van het bericht
        const data = JSON.parse(event.data);
        if (data && data.checkAvailableDates) {
            console.log('Received checkAvailableDates:', data.checkAvailableDates);
            window.checkAvailableDates = data.checkAvailableDates;
        }
    } catch (e) {
