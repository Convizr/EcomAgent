export const DatePickerExtension = {
    ...
    render: ({ trace, element }) => {
      let payloadObj;
      // First, determine if trace.payload is a stringified JSON and parse if necessary
      if (typeof trace.payload === 'string') {
          payloadObj = JSON.parse(trace.payload);
      } else {
          // If not a string, use directly
          payloadObj = trace.payload;
      }

      // Extract 'checkAvailableDates' as a string and split it into an array
      ...
      // Now, availableDates is an array that can be used to populate the dropdown
