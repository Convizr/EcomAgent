export const DatePickerExtension = {
    ...
    render: ({ trace, element }) => {
      // Correct extraheren van de checkAvailableDates waarde uit de payload
      let checkAvailableDates = trace.payload;
      console.log('Available dates string:', checkAvailableDates);
  
      // Veronderstelt dat checkAvailableDates een string is met datums gescheiden door komma's
      // Eerst controleren we of checkAvailableDates daadwerkelijk een string is en splitsen we deze vervolgens in een array van datums
      const availableDates = typeof checkAvailableDates === 'string' ? checkAvailableDates.split(',').map(date => date.trim()) : [];
