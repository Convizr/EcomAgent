...
export const TimePickerExtension = {
    ...
    render: ({ trace, element }) => {
        // Assuming 'availableTimes' is passed in the trace payload and is a JSON string.
        let availableTimesJsonString = trace.payload;
        let availableTimes = [];

        try {
            // Parse the JSON string to get the array of available time slots.
            availableTimes = JSON.parse(availableTimesJsonString);
        } catch (error) {
            console.error('Error parsing available times:', error);
            // Handle error or set a fallback for availableTimes
        }
